...
import SendPasswordPage from "./SendPasswordPage";
function Form({ changeForm, closePopup, changeModalContent }:{
  ...
  let currentPage:string[] = path.pathname.split("/");
  let currPg:string = currentPage[currentPage.length - 1];

  function authenticateUser(e:FormEvent) {
    e.preventDefault();
    const auth = getAuth();
